...
class SuratmasukController extends Controller
{
    ...
    //fumction edit dokumen saja
    public function update(Request $request, $id)
{
    $suratmasuk = SuratMasuk::findOrFail($id);

    // Validasi formulir, termasuk validasi file jika diperlukan
    $request->validate([
        'filemasuk' => 'mimes:pdf,doc,docx|max:2048', // Contoh validasi untuk tipe dan ukuran file tertentu
        // ... validasi lainnya ...
    ]);
    ...
    if ($request->hasFile('filemasuk')) {
        ...
        $file->storeAs('suratMasuk', $fileName, 'public'); // Ganti 'folder_tujuan' dengan direktori penyimpanan yang diinginkan

        // Update nama file baru di dalam database
        $suratmasuk->filemasuk = $fileName;
    }

    // ... proses penyimpanan dan pembaruan data lainnya ...

    return redirect()->back()->with('sukses', 'File berhasil diperbarui');
}
